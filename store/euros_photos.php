...
foreach($articles as $article)	
{
	if ($elements[$article])
	{
		...
		$img = CFile::GetPath($elements[$article]["DETAIL_PICTURE"]);
		...
		copy($_SERVER['DOCUMENT_ROOT'].$img, $_SERVER['DOCUMENT_ROOT'].'/store/pictures/'.$elements[$article]["PROPERTY_CML2_ARTICLE_VALUE"].'.'.$ext);
		//echo $_SERVER['DOCUMENT_ROOT'].$img.'<br>';
		echo 'https://www.termoros.com/store/pictures/'.$elements[$article]["PROPERTY_CML2_ARTICLE_VALUE"].'.'.$ext.'<br>';
	}
	else
	{
		echo '<br>';
