...
function isEmail ($email) {
   if(preg_match("/^([a-z0-9_-]+\.)*[a-z0-9_-]+@[a-z0-9_-]+(\.[a-z0-9_-]+)*\.[a-z]{2,6}$/i",$email)) return true;
else return false;
}



//var_dump(verifyEmail('lizesrgesrgotfyjtfyjtfjycom2@mail.ru', 'lizocom2@mail.ru', true));
...
while($buffer = fgets($fh, 1024))
{
	$k++;
	$buffer = trim($buffer);
	var_dump(verifyEmail($buffer, 'info@termoros.com', true));
	echo '<br>';
