...
if($orders)
{
	$res = \CSaleOrderPropsValue::GetList([],['CODE'=>'EMAIL','ORDER_ID'=>$orders]);
	while($ar = $res->Fetch())
	{
		$arResult['EMAILS'][$ar['ORDER_ID']] = $ar['VALUE'];
		//p($ar);
	}

	$res  = \CSaleOrderUserProps::GetList([],['USER_ID'=>$USER->GetID()]);
	while($profile = $res->Fetch())
	{
		$ar = \CSaleOrderUserPropsValue::GetList([],['USER_PROPS_ID'=>$profile['ID'],'CODE'=>'EMAIL'],false,false)->Fetch();
		$arResult['PROFILES'][$ar['VALUE']] = $profile['NAME'];
	}
}
