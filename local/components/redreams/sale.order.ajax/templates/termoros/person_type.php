...
<?if(count($arResult["PERSON_TYPE"]) > 1)
{
	...
	<div class='choose_profile'>
		<?foreach($arResult["PERSON_TYPE"] as $v):?>
				<label class="col" for="PERSON_TYPE_<?=$v["ID"]?>"><input class="customRadio" type="radio" id="PERSON_TYPE_<?=$v["ID"]?>" name="PERSON_TYPE" value="<?=$v["ID"]?>"<?if ($v["CHECKED"]=="Y") echo " checked=\"checked\"";?> onClick="submitForm();"> <?=$v["NAME"]?></label>
				<?if ($v["CHECKED"]=="Y") $ptID = $v["ID"];?>
			<?endforeach;?>
