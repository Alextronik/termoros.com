		...
		if (!empty($arSource))
		{
			...
			<?
					foreach ($arSource as $arProperties)
					{
						...
						?>
						<div class="inpt"  data-property-id-row="<?=intval(intval($arProperties["ID"]))?>" <?if(($arProperties["TYPE"] == "LOCATION" && $arProperties["ID"] != "6") || $arProperties["CODE"] == "CITY"):?>style="display: none;"<?endif;?>  >
						...
						if ($arProperties["TYPE"] == "CHECKBOX")
						{
							...
							<input data-error_mes="Для заказа вы должны подтвердить свой возраст!"  type="checkbox" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>" <?=(in_array($arProperties["FIELD_NAME"],$arrValid)||$arProperties["REQUIED_FORMATED"]=="Y")?'data-req="1"':""?> class="customCheckbox"  value="Y"<?if ($arProperties["CHECKED"]=="Y") echo " checked";?>>
							<label for="<?=$arProperties["FIELD_NAME"]?>">мне более 18 лет <?=$arProperties["REQUIED_FORMATED"]=="Y"?"*":""?></label>
							<?
						}
						...
						elseif ($arProperties["TYPE"] == "LOCATION")
						{
							...
							<div class="bx_block r3x1"  >
								...
								<?CSaleLocation::proxySaleAjaxLocationsComponent(array(
									...
									"CITY_OUT_LOCATION" => "Y",
									"LOCATION_VALUE" => ($value)?$value:$_SESSION['GEOIP']['curr_city_id'], //$value ?
									"ORDER_PROPS_ID" => $arProperties["ID"],
									"ONCITYCHANGE" => ($arProperties["IS_LOCATION"] == "Y" || $arProperties["IS_LOCATION4TAX"] == "Y") ? "submitForm()" : "",
									"SIZE1" => $arProperties["SIZE1"],
								),
								array(
									"ID" => ($value)?$value:$_SESSION['GEOIP']['curr_city_id'],//$value,
									"CODE" => "",
