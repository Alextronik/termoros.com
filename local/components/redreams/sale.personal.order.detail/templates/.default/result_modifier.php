...
if($arProducts)
{
	$arSelect = Array("NAME","ID","PROPERTY_CML2_ATTRIBUTES","IBLOCK_ID","CATALOG_QUANTITY");
	...
	while($obRes = $res->GetNextElement())
	{
		...
		if($_SESSION['STORES'][$_SESSION['GEOIP']['curr_city_name']]['ID']){
			...
			$rsStore = CCatalogStoreProduct::GetList(array(), array('PRODUCT_ID' => $arFields['ID'], 'STORE_ID' => $_SESSION['STORES'][$_SESSION['GEOIP']['curr_city_name']]['ID']),false, false, array('PRODUCT_ID', 'STORE_ID', 'STORE_NAME', 'AMOUNT'));
