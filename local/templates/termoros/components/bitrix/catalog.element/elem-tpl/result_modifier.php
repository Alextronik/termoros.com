...
//p($_SESSION['STORES'][$_SESSION['GEOIP']['curr_city_name']]['ID']);
	...
	$rsStore = CCatalogStoreProduct::GetList(array(), array('PRODUCT_ID' => $arResult['ID'], 'STORE_ID' => $_SESSION['STORES'][$_SESSION['GEOIP']['curr_city_name']]['ID']), false, false, array('PRODUCT_ID', 'STORE_ID', 'STORE_NAME', 'AMOUNT', 'STORE_ADDR'));
	while($arStore = $rsStore->Fetch()){
		$arResult['QUANTS'][$arStore['PRODUCT_ID']] = $arStore['AMOUNT'];
		$arResult["STORE_ADDRESS"] = $arStore['STORE_ADDR'];
	}

	$rsStore = CCatalogStoreProduct::GetList(array("STORE_ADDR" => "ASC"), array('PRODUCT_ID' => $arResult['ID'], '!STORE_ID' => $_SESSION['STORES'][$_SESSION['GEOIP']['curr_city_name']]['ID']), false, false, array('PRODUCT_ID', 'STORE_ID', 'STORE_NAME', 'AMOUNT', 'STORE_ADDR'));
	while($arStore = $rsStore->Fetch()){
		if ($arStore['AMOUNT'])
		{
			$arResult['OTHER_STORES'][] = $arStore;
		}
	}
	//p($arResult['OTHER_STORES']);
...
//p($arcat['MEASURE']);
if($arcat['MEASURE']&&!$arResult["PROPERTIES"]["CML2_BASE_UNIT"]["VALUE"]) $arResult["PROPERTIES"]["CML2_BASE_UNIT"]["VALUE"] = $measures[$arcat['MEASURE']];
//p($arResult["ID"]);
//p($arcat);
...
if($arResult['PROPERTIES']['BREND']['VALUE']){
	...
	$arFilter = array("UF_XML_ID" => $arResult['PROPERTIES']['BREND']['VALUE']);
	...
	// Заглушка для Gekon
	if (substr($arRes['UF_NAME'],0,5) == "Gekon") $arRes['UF_NAME'] = "Gekon";
	
	$arSelect = Array('IBLOCK_ID','NAME','ID','DETAIL_PICTURE', 'DETAIL_PAGE_URL');
	$arFilter = Array("IBLOCK_ID"=>17, "ACTIVE"=>"Y", "NAME" => $arRes['UF_NAME']);
	...
	if (!$fields)
	{
		...
		$arSelect = Array('IBLOCK_ID','NAME','ID','DETAIL_PICTURE', 'DETAIL_PAGE_URL');
		$arFilter = Array("IBLOCK_ID"=>17, "ACTIVE"=>"Y", "CODE" => $arRes['UF_NAME']);
		$res = CIBlockElement::GetList(Array(), $arFilter, false, Array("nPageCount"=>1), $arSelect);
		while($ob = $res->GetNextElement())
		{
			$fields = $ob->GetFields();
	...
	/*$arResult['BREND'] = array(
		"NAME" => $arRes['UF_NAME'],
		"LINK" => "/brands/".str_replace(".", "", strtolower(trim($arRes['UF_NAME'])))."/",
		"PIC" => CFile::ResizeImageGet($arRes['UF_FILE'], array('width'=>240, 'height'=>240), BX_RESIZE_IMAGE_PROPORTIONAL  , true),
	);*/
	$arResult['BREND'] = array(
		"NAME" => $fields['NAME'],
		"LINK" => $fields['DETAIL_PAGE_URL'],
		"PIC" => CFile::ResizeImageGet($fields['DETAIL_PICTURE'], array('width'=>240, 'height'=>60), BX_RESIZE_IMAGE_PROPORTIONAL  , true),
	);
	//p($arResult['BREND']);
}
...
Redreams\Partners\discounts::processPartnerDiscount($arResult);
...
foreach ($_SESSION['STORES'] as $store)
{
	...
	{
		$arResult["SUPPLY"] = SU\SupplyInfo::GetSupply($arResult["ID"], $store["ID"]);
	}
}
