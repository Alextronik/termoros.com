...
?>
	...
	<? } ?>
	<?if ($arResult['FORM_ERRORS']["THEMES"]) { ?>
		<p style="color: red;"><?=$arResult['FORM_ERRORS']["THEMES"]?></p>
	<? } ?>
	<?=$arResult['FORM_HEADER']?>
	<div class="pop_left inzhiniring_form">
			...
			<?//p($arQuestion)?>
				...
				<?if($arQuestion['STRUCTURE'][0]['FIELD_TYPE']=='checkbox'):?>				
					...
					<div class="inpt custm" >
						<p class='inp_name'><?=$arQuestion['CAPTION']?></p>
						<?=str_replace("inputtext", "inp_self formfield", $arQuestion['HTML_CODE']);?>
				...
				<div class='inpt custm'>
					...
					<p class='sub'>Размер загружаемого файла не должен превышать 5мб</p>
	...
	<?// if ($USER->IsAdmin()) { ?>
		...
		<p><input data-req="1" class="policy_agree" name="policy_agree" checked="checked" value="1" type="checkbox"> <a style="color: red;" target="_blank" href="/copy.php"> Согласен на обработку персональных данных</a></p>
	<?// } ?>
	...
	<script>
		...
		$(document).ready(function() {
			if ($('input[name="form_text_1416"]').length)
			{
				$('input[name="form_text_1416"]').hide();
				$('input[name="form_text_1417"]').hide();
				$('.inpt.custm label').css('max-width', 850);
				$('.inpt.custm label').parent().css('width', 900);
