...
<div class='shops_wp'>
		...
		<form method="get" id="a_filter">
			...
			<? if ($arResult['METRO'] && $arResult['METRO'][0]): ?>
				<select class='customSelect mtr sort_sel' name="metro"
						action="<?= $APPLICATION->GetCurPageParam("", array("")); ?>">
					<option selected="selected" value="">Выберите метро</option>
			...
			<ul class='shop_list'>
				...
				<? foreach ($arResult["ITEMS"] as $cell => $arElement): ?>
					<li>
						...
						if ($arElement["PREVIEW_PICTURE"])
						{
							echo '<p><a target="_blank" rel="img_'.$arElement["PREVIEW_PICTURE"]["ID"].'" class="fancyPrint" href="'.$arElement["PREVIEW_PICTURE"]["SRC"].'">Схема проезда на общественном транспорте</a></p>';
						}
						
						if ($arElement["DETAIL_PICTURE"])
						{
							echo '<p><a target="_blank" rel="img_'.$arElement["DETAIL_PICTURE"]["ID"].'" class="fancyPrint" href="'.$arElement["DETAIL_PICTURE"]["SRC"].'">Схема проезда на автомобиле</a></p>';
						}
						*/
						if ($arElement['PROPERTIES']["SCHEME"]["VALUE"])
						{
	...
	<? if ($arParams['SECTS'] && !$arParams['BIGSCALE']) {?>
		<div class="citys_list"  style="-moz-column-count: 5;-moz-column-gap: 1rem;-webkit-column-count: 5;-webkit-column-gap: 1rem;column-count: 5;column-gap: 1rem;">
            <? foreach ($arParams['SECTS'] as $k=>$v) {?>
                 <a href='?city=<?= $v; ?>' class='name'><?=$k?></a>
			<?}?>
		</div>
	<?}?>
</div>
