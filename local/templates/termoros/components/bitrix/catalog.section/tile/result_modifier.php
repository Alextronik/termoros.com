...
if (!empty($arResult['ITEMS']))
{
	...
	$strEmptyPreview = SITE_TEMPLATE_PATH."/img/no-foto-small.jpg";
...
foreach($_SESSION['CATALOG_COMPARE_LIST'][4]['ITEMS'] as $compare_id => $compare_arr){
	$compares[] = $compare_id;
}

$arResult['COMPARE'] = $compares;
$arResult['FAV'] = getFav();
...
while($arRes = $rsData->Fetch()){
	//p($arRes);
	$brands[$arRes['UF_XML_ID']] = $arRes['UF_NAME'];
}

$arResult['BRANDS'] = $brands;


$arSelect = Array('IBLOCK_ID','NAME','DETAIL_PAGE_URL');
...
$arResult['BRANDSBYNAME'][strtolower($arBrends['NAME'])] = $arBrends['DETAIL_PAGE_URL'];
}
...
foreach ($_SESSION['STORES'] as $store)
{
	if($store["CITY_ID"] == $curCityId)
	{
		...
		{
			$arResult['ITEMS'][$k]["SUPPLY"] = SU\SupplyInfo::GetSupply($arItem["ID"], $store["ID"]);
		}
	}
}
*/
...
if($_SESSION['STORES'][$_SESSION['GEOIP']['curr_city_name']]['ID']){
	...
	foreach($arResult['ITEMS'] as $k => $arItem)
	{
		$rsStore = CCatalogStoreProduct::GetList(array(), array('PRODUCT_ID' => $arItem['ID'], 'STORE_ID' => $_SESSION['STORES'][$_SESSION['GEOIP']['curr_city_name']]['ID']), false, false, array('PRODUCT_ID', 'STORE_ID', 'STORE_NAME', 'AMOUNT'));
		while($arStore = $rsStore->Fetch()){
			//p($arStore['AMOUNT']); 
			$arResult['ITEMS'][$k][$_SESSION['STORES'][$_SESSION['GEOIP']['curr_city_name']]['ID']]['QUANTS'] = $arStore['AMOUNT'];
		}
		$arcat = CCatalogProduct::GetByID($arItem['ID']);
		//p($arcat['MEASURE']);
		if($arcat['MEASURE']&&!$arItem["PROPERTIES"]["CML2_BASE_UNIT"]["VALUE"]) $arResult['ITEMS'][$k]["PROPERTIES"]["CML2_BASE_UNIT"]["VALUE"] = $measures[$arcat['MEASURE']];
	}

}

//p($arResult['BRANDSBYNAME']);
?>