...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		getProfilesControl: function(node)
		{
			...
			var partnerProfilesCount = 0;
			
			if (profilesLength)
			{
				if (
					this.params.ALLOW_USER_PROFILES === 'Y'
					&& (profilesLength > 0 || this.params.ALLOW_NEW_PROFILE === 'Y')
				)
				{
					this.regionBlockNotEmpty = true;
					...
					for (i in this.result.USER_PROFILES)
					{
						if (this.result.USER_PROFILES.hasOwnProperty(i))
						{
							...
							//dt = new Date(this.result.USER_PROFILES[i].DATE_UPDATE);
							
							if (this.params.IS_PARTNER == 'Y')
							{
								
								var dateArr = this.result.USER_PROFILES[i].DATE_UPDATE.split(" ")[0].split(".");
								
								var updDate = 0;
								var nowDate = 0;
								updDate = Date.parse(dateArr[2]+'-'+dateArr[1]+'-'+dateArr[0]);
								nowDate = Date.now();
								if ((nowDate - 60*60*24*5*1000) <= updDate)
								{
								...
								//console.log(partnerProfilesCount);
								/*
								if (this.params.IS_PARTNER == 'Y' && partnerProfilesCount == 0)
								{
									//this.showError(node, '123')
								}
								*/
			...
			else
			{
				...
				if (this.params.IS_PARTNER == 'Y')
				{
					//this.result.order.ERROR.push('У вас нет доступных контрагентов на этом типе плательшика. Чтобы загрузить контрагентов на сайт обратитесь к Вашему менеджеру.');
					//this.showErrors({"MAIN": 'У вас нет доступных контрагентов на этом типе плательшика. Чтобы загрузить контрагентов на сайт обратитесь к Вашему менеджеру.'}, true, true);
				}
				/*
				node.appendChild(BX.create('B', {
					html: 'У вас нет доступных контрагентов на этом типе плательшика. Чтобы загрузить контрагентов на сайт обратитесь к Вашему менеджеру.'
				}))
				*/
			}
		...
		isValidRegionBlock: function()
		{
			...
			//Нет профиля для партнера и нет возможности его создать + отсеиваем профили, которые не обновлялись более 5и дней
			if (this.params.IS_PARTNER == 'Y')
			{
				var profilesLength = BX.util.object_keys(this.result.USER_PROFILES).length;
				...
				if (profilesLength == 0)
				{
					regionErrors.push('У вас нет доступных контрагентов на этом типе плательшика. Чтобы загрузить контрагентов на сайт обратитесь к Вашему менеджеру.');
				}
				else
				{
					var partnerProfilesCount = 0
