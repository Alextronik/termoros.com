...
<script type="text/javascript">
	function fShowStore2(id, showImages, formWidth, siteId)
	{
		...
		?>
					...
					<div class='dt_list'>
						<div class='dt_item lbl'>
							<input
								...
								id="ID_DELIVERY_<?=$delivery_id?>_<?=$profile_id?>_2"
								name="<?=htmlspecialcharsbx($arProfile["FIELD_NAME"])?>_2"
								value="<?=$delivery_id.":".$profile_id;?>"
								<?=$_REQUEST[$arProfile["FIELD_NAME"].'_2'] == $delivery_id.":".$profile_id ? "checked=\"checked\"" : "";?>
								onclick="submitForm();"
								/>
							<label for="ID_DELIVERY_<?=$delivery_id?>_<?=$profile_id?>_2">
			...
			else // stores and courier
			{
				...
				if (count($arDelivery["STORE"]) > 0)
					...
					$clickHandler = "onClick = \"BX('ID_DELIVERY_ID_".$arDelivery["ID"]."_2').checked=true;submitForm();\"";
				?>
					<?if ($_REQUEST[$arDelivery["FIELD_NAME"]."_2"]==$arDelivery["ID"] || !$_REQUEST[$arDelivery["FIELD_NAME"]."_2"]&&$arDelivery["ID"]==1) {$arDelivery["CHECKED"] = "Y";}else{$arDelivery["CHECKED"] = "N";};?>

					<?if ($arDelivery["CHECKED"]=="Y") :?>
						<?$deliver_price_2 = $arDelivery["PRICE"]?>
					<?endif?>
					...
					<div class='dt_list'>
						<div class='dt_item lbl'>
							<input type="radio" class="customRadio"
								id="ID_DELIVERY_ID_<?= $arDelivery["ID"] ?>_2"
								name="<?=htmlspecialcharsbx($arDelivery["FIELD_NAME"])?>_2"
								value="<?= $arDelivery["ID"] ?>"<?if ($arDelivery["CHECKED"]) echo " checked";?>
								onclick="submitForm();"
								/>
							<label for="ID_DELIVERY_ID_<?= $arDelivery["ID"] ?>_2" <?=$clickHandler?>>
						...
						<div class='dt_item'>
							...
							?>
								...
								<a href='' onclick="fShowStore('2','N','700','s1');" class='lnk'>пункты самовывоза</a>
							<?
							endif;
							?>
