...
if (!function_exists("PrintPropsForm2"))
{

	function PrintPropsForm2($arSource = array(), $locationTemplate = ".default",$group_ID)
	{
		$arrValid=array(
			"ORDER_PROP_1",
			"ORDER_PROP_20",
			...
			<?
					...
					foreach ($arSource as $arProperties)
					{
						...
						if($arProperties["VALUE"]==""){
							if($arrSoot[$arProperties["ID"]]){
								$arProperties["VALUE"]=$arSource[$arrSoot[$arProperties["ID"]]]["VALUE"];
							}
						}
						...
						if ($arProperties["TYPE"] == "CHECKBOX")
						{
							...
							<input  type="checkbox" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>" class="customCheckbox"  value="Y"<?if ($arProperties["CHECKED"]=="Y") echo " checked";?>>
						...
						{
							?>
							<input type="text" class="inp_self <?=$sh>2?"short":""?> <?=$sh==5?"last":""?>" value="<?=$arProperties["VALUE"]=="<Без имени>"?"":$arProperties["VALUE"]?>" maxlength="250" size="<?=$arProperties["SIZE1"]?>" <?=in_array($arProperties["FIELD_NAME"],$arrValid)?'data-req="1"':""?> name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>" placeholder="<?=$arProperties["NAME"]?> <?=$arProperties["REQUIED_FORMATED"]=="Y"?"*":""?>">
