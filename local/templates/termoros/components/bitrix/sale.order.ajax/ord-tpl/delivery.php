...
<div class='city_wp'>
	<p class='name'>Ваш город<span>МОСКВА</span></p>
	<a href='' onclick="$('.loc-contain').show();$(this).hide();$(this).prev().hide(); return false;">изменить</a>
	<div class="loc-contain" style="display: none" > 
	<?=PrintPropsForm($arResult["ORDER_PROP"]["USER_PROPS_Y"], $arParams["TEMPLATE_LOCATION"], false, false, true); // location?>
	</div>
	<?//p($arResult["ORDER_PROP"]);?>
</div>
...
<input type="hidden" name="BUYER_STORE" id="BUYER_STORE" value="<?=$arResult["BUYER_STORE"]?>" />
	...
	if(!empty($arResult["DELIVERY"]))
	{
		...
		foreach ($arResult["DELIVERY"] as $delivery_id => $arDelivery)
		{
			...
			?>
			<div class='order_type_wp <?if ($arDelivery["CHECKED"]=="Y") echo "active";?>'>
				...
				<div class='cost_wp'>
					...
					if (count($arDelivery["STORE"]) > 0):
						?>
							<span class='self' id="select_store"<?if(strlen($arResult["STORE_LIST"][$arResult["BUYER_STORE"]]["TITLE"]) <= 0) echo " style=\"display:none;\"";?>>
							...
							<a href='' class='self'>пункты самовывоза</a>
						<?
					endif;
					?>
				</div>
				...
				<div class='adr_block' >
					<?if ($arDelivery["CHECKED"]=="Y"):?>
						...
						<p class='ttl'>Введите адрес доставки</p>
						<?=PrintPropsForm($arResult["ORDER_PROP"]["RELATED"], $arParams["TEMPLATE_LOCATION"], false, false, false, false)?>
						<div class='clear'></div>
						<?endif;?>
					<?endif;?>
				</div>
