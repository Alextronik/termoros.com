...
{
	$fh = fopen($_SERVER['DOCUMENT_ROOT'].'/api/search/logs/search_log', 'a+');
	fwrite($fh, $searchStr."\n");
	fclose($fh);
}
*/
				...
				if(substr($arItem["ITEM_ID"], 0, 1) !== "S")
				{
					...
					$rsElement = CIBlockElement::GetList(array(), array(
						...
						false, false, array(
							...
							"PROPERTY_BREND",
							"PROPERTY_ARTIKUL_NOMENKLATURY_POSTAVSHCHIKA",
						)
					);
					...
					if($_SESSION['STORES'][$_SESSION['GEOIP']['curr_city_name']]['ID'])
					{
						...
						$rsStore = CCatalogStoreProduct::GetList(array(), array('PRODUCT_ID' => $arElement['ID'], 'STORE_ID' => $_SESSION['STORES'][$_SESSION['GEOIP']['curr_city_name']]['ID']), false, false, array('PRODUCT_ID', 'STORE_ID', 'STORE_NAME', 'AMOUNT'));
						while($arStore = $rsStore->Fetch()){
							$arElement['QUANTS'] = $arStore['AMOUNT'];
						}
					...
					$arcat = CCatalogProduct::GetByID($arResult['ID']);
					//p($arcat['MEASURE']);
					if($arcat['MEASURE']&&!$arElement["PROPERTIES"]["CML2_BASE_UNIT"]["VALUE"]) $arElement["PROPERTIES"]["CML2_BASE_UNIT"]["VALUE"] = $measures[$arcat['MEASURE']];
					
					
					//if(stripos($arElement['NAME'],$arElement['PROPERTY_CML2_ARTICLE_VALUE'])===false && $arElement['PROPERTY_CML2_ARTICLE_VALUE'])
					$postavschik = '';
					if($arElement['PROPERTY_ARTIKUL_NOMENKLATURY_POSTAVSHCHIKA_VALUE'])
					{
						$postavschik = '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Артикул поставщика: '.$arElement['PROPERTY_ARTIKUL_NOMENKLATURY_POSTAVSHCHIKA_VALUE'];
					}
				
					if($arElement['PROPERTY_CML2_ARTICLE_VALUE'])
					{
						if($arElement['PROPERTY_BREND_VALUE'])
						{
							...
							$brand = BrendTable::getList(['filter'=>['UF_XML_ID' => $arElement['PROPERTY_BREND_VALUE']]])->fetch();
							$brandName = $brand["UF_NAME"];
							//$arItem['NAME'] = $arElement['NAME'].'<br><p style="font-size:11px; padding: 0; margin-top: -2px;">Артикул: '.$arElement['PROPERTY_CML2_ARTICLE_VALUE'].'</p>';
						}
						...
						if ($isPartner)
						{
							if ($arElement['QUANTS'])
							{
								$quantity = '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Наличие: '.$arElement['QUANTS'].' '.$arElement["PROPERTIES"]["CML2_BASE_UNIT"]["VALUE"];
							}
							else
							{
								$quantity = '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Наличие уточняйте у менеджеров';
							}
						}
						
						$arItem['NAME'] = $arElement['NAME'].'<br><p style="font-size:11px; padding: 0; margin-top: -2px;">Бренд: '.$brandName.'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Артикул: '.$arElement['PROPERTY_CML2_ARTICLE_VALUE'].$quantity.$postavschik.'</p>';
					}
					
					
					
					$striposRes = stripos($arItem['NAME'],$arResult["query"]);
					if ($striposRes!==false)
					{
						$tmpName = substr($arItem["NAME"], $striposRes, strlen($arResult["query"]));
						
						$arItem['NAME'] = str_replace($tmpName, '<b>'.$tmpName.'</b>', $arItem['NAME']);
					}
