...
?><div class="bx_sort_container">
	<?//p($arResult['COMPARE_URL_TEMPLATE']);?>
	<div class="sorttext"><?=GetMessage("CATALOG_SHOWN_CHARACTERISTICS")?>:</div>
	<a class="sortbutton <?if(!$_REQUEST['DIFFERENT']||$_REQUEST['DIFFERENT'] == 'N'):?>current<?endif?><?// echo (!$arResult["DIFFERENT"] ? ' current' : ''); ?>" href="/personal/compare?DIFFERENT=N<?// echo $arResult['COMPARE_URL_TEMPLATE'].'DIFFERENT=N'; ?>" rel="nofollow"><?=GetMessage("CATALOG_ALL_CHARACTERISTICS")?></a>
	<a class="sortbutton <?if($_REQUEST['DIFFERENT'] == 'Y'):?>current<?endif?><?// echo ($arResult["DIFFERENT"] ? ' current' : ''); ?>" href="/personal/compare?DIFFERENT=Y<?// echo $arResult['COMPARE_URL_TEMPLATE'].'DIFFERENT=Y'; ?>" rel="nofollow"><?=GetMessage("CATALOG_ONLY_DIFFERENT")?></a>
</div>
<?
				...
				switch($code)
				{
					...
					case "DETAIL_PICTURE":
						if(is_array($arElement["FIELDS"][$code])):?>
							<a class="compare-pic" href="<?=$arElement["DETAIL_PAGE_URL"]?>">
							<img
							border="0"
...
if (!empty($arResult["SHOW_OFFER_PROPERTIES"]))
{
	...
	<tr>
		...
		?>
		<td>
			<a class="compare-del" onclick="CatalogCompareObj.MakeAjaxAction('<?=CUtil::JSEscape($arElement['~DELETE_URL'])?>');" href="javascript:void(0)"><?=GetMessage("CATALOG_REMOVE_PRODUCT")?></a>
			<a href="" class="to_basket ajax list">в корзину</a>
			<form name="fastfind" style="display: none;">
				<input name="ID" value="<?=$arElement['ID']?>" type="hidden">
				<input name="id" value="<?=$arElement['ID']?>" type="hidden">
				<input name="IBLOCK_ID" value="<?=$arElement['IBLOCK_ID']?>" type="hidden">
			</form>
		</td>
		<?
		}
