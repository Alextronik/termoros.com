	...
	switch ($arParams['VIEW_MODE'])
	{
		...
		case 'TILE':
			...
			foreach ($arResult['SECTIONS'] as &$arSection)
			{
				...
				?>
				<div class='mc_item col-3 py-3' id="<? echo $this->GetEditAreaId($arSection['ID']); ?>">

					<div class='mc_name'>
						<div class='im_area'>
							<img src='<?=$arSection['PICTURE']["SRC"]?>'>
						</div>
						<a href='<? echo $arSection['SECTION_PAGE_URL']; ?>' class='name'><? echo $arSection['NAME']; ?></a>
					</div>

					<ul>
						<?$count=0;?>
						<?foreach($arSection["SECTION_VIST"] as $arPodSection){?>
							<li><a href='<?=$arPodSection["SECTION_PAGE_URL"]?>'><?=$arPodSection["NAME"]?></a></li>
							<?$count++;
							if($count==4)
								break;
							?>
					...
					<div class='more_wp'>
						<a href='<? echo $arSection['SECTION_PAGE_URL']; ?>' class='mc_more'></a>
					</div>
