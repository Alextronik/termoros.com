...
{	
	/*$.ajax({
		type: 'POST',
		url: '/bitrix/templates/.default/ajax/scripts/check_coupon.php',
		data: 'COUPON='+$('.promo_input').val(),
		async: false,
		success: function(html) {
			if(html=='ok')
			{
				
				$('.promo_input').addClass('good');
				flag = true;	
				updateBigBasket();	
			}
			else
			{
				...
				$('.discount .errorreport').show();
				$('.discount .coupon_yes').hide();
				$('.promo_input').addClass('bad');
				flag = false;
			}
...
function updateBigBasket()
{
	...
	xhr = $.ajax({
		type: 'POST',
		data: data+'&ajax_basket=y&BasketRefresh=y',
		success: function(html) {
			if(html)
			{
				//console.log(html);
				basket = $(html).find('#basket_form_container').html();			
				$('#basket_form_container').html(basket);
				customInput();
				//refreshBasket();
				//location.reload();
