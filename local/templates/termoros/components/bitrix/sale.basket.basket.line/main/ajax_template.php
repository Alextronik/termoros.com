		...
		<div id="<?=$cartId?>products" class="bx-basket-item-list-container">
			<?foreach ($arResult["CATEGORIES"] as $category => $items):
				if (empty($items))
					continue;
				?>
				<div class="bx-basket-item-list-item-status"><a onclick="yaCounter26951046.reachGoal('pereiti_korzina'); ga('send', 'pageview','/virtual/pereiti_korzina'); return true;" href="/personal/cart" class="go_bask">перейти В корзину</a></div>
				<?foreach ($items as $v):
                $art = false;
                $db_props = CIBlockElement::GetProperty(4, $v['PRODUCT_ID'], array(), Array("CODE"=>"CML2_ARTICLE"));
                ...
                ?>
					<div class="bx-basket-item-list-item">
						...
						<div class="bx-basket-item-list-item-img">
                            <?if(empty($v["PICTURE_SRC"])) $v["PICTURE_SRC"] = SITE_TEMPLATE_PATH."/img/no-foto-small.jpg"?>
                        ...
                        <div class="bx-basket-item-list-item-art">
                            <?if ($art){?>
                                <p>Артикул: <?=$art?></p>
                            <?}?>
                        </div>
