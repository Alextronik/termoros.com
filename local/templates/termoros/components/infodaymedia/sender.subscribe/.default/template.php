...
?>
	...
	<form role="form" method="post" action="<?=$arResult["FORM_ACTION"]?>"  onsubmit="BX('bx_subscribe_btn_<?=$buttonId?>').disabled=true;">
		...
		<div class="subs_wp">
			...
			<input type='text' class='inp_self' name="SENDER_SUBSCRIBE_EMAIL" value='<?=($arResult["EMAIL"])?$arResult["EMAIL"]:$_COOKIE['BITRIX_SM_SENDER_SUBSCR_EMAIL']?>' placeholder='Ваш e-mail'>
<input type='submit' id="bx_subscribe_btn_<?=$buttonId?>" class='btn' value='Подписаться' >	
		</div>
		
		<?foreach($arResult["RUBRICS"] as $itemID => $itemValue):?>
		<div class="bx_subscribe_checkbox_container" style="display: none;">
			<input type="checkbox" name="SENDER_SUBSCRIBE_RUB_ID[]" id="SENDER_SUBSCRIBE_RUB_ID_<?=$itemValue["ID"]?>" value="<?=$itemValue["ID"]?>"<? echo " checked"?>>
